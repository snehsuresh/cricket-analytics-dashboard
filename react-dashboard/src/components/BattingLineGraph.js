...
const LineGraph = ({ inningsData, currentInnings }) => {
    ...
    const updateBattingPerformance = () => {
        ...
        const svgWidth = 900;  // Increased width to ensure legend is fully visible
        const svgHeight = 400;
        const margin = { top: 20, right: 150, bottom: 50, left: 50 };  // Adjusted margin to make more space for the legend
        ...
        // Add a legend outside the graph, to the right
        const legend = svg.append("g")
            .attr("transform", `translate(${svgWidth - margin.right + 30}, ${margin.top})`); // Adjusted legend position slightly
        ...
        uniqueBatsmen.forEach((batsman, index) => {
            ...
            legend.append("text")
                ...
                .attr("y", index * 20 + 12)
                .text(batsman)
                .style("font-size", "12px"); // Optional: reduce font size if needed to fit better
        });
